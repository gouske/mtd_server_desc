...
> **영상**: [OBT트레일러](https://youtu.be/EoZhHSy1ggk?si=E1FY6DYnMeSCxWeZ), [플레이영상1](https://youtu.be/XouaK8bREkM?si=iANHVgYQgV1dumH0), [플레이영상2](https://youtu.be/UhZ8D-zk3dc?si=r8IdbK5BzdRhygKd)
...
| **데이터베이스** | MySQL | 유저 정보, 아이템, 게임 기록 등 영구 저장. 읽기/쓰기 서버 분리, 3개 게임 서버 샤딩 |
| **빠른 캐시 · 랭킹** | Redis | 자주 쓰는 데이터 메모리 저장, 아레나 순위 실시간 집계 (Sorted Set · Hash · Lua 스크립트) |
...
| 역할 | 기술 | 한 줄 설명 |
|------|------|-----------|
| **NFT/블록체인** | Marblex API | NFT 거래 및 블록체인 연동 |
| **소셜 로그인** | Google / Apple OAuth | 구글·애플 계정으로 로그인 |
| **NFT 발행** | Immutable X (IMX) | IMX 기반 NFT 발행 처리 |

### 핵심 설계 기법

| 분류 | 기법 | 목적 |
|------|------|------|
| **안정성** | 재시도 간격 분산(Jitter) | DB 충돌 자동 복구 |
| **안정성** | 확률적 조기 갱신(XFetch) | 캐시 만료 시 동시 과부하 방지 |
| **안정성** | Redis 분산 잠금 | 랭킹 보드 중복 갱신 방지 |
| **일관성** | 쓰기 후 300ms Writer 고정 | DB 복제 지연 데이터 불일치 방지 |
| **설계** | 커밋 시 도메인 훅 자동 실행 | 앱 응답 데이터 누락 방지 |
| **설계** | 응답 후 일괄 로그 기록 | 로그 쓰기가 응답 속도에 영향 없도록 |
| **설계** | 객체 소멸 시 자동 트랜잭션 취소 | 롤백 누락 방지 |
| **성능** | Dirty Flag 기반 스탯 재계산 | 전투 중 불필요한 연산 최소화 |
...
| # | 업무 | 해결한 문제 | 결과 |
|---|------|------------|------|
| 1 | DB 읽기/쓰기 자동 분리 라우팅 | 복제 지연으로 인한 데이터 불일치 | 불일치 오류 제거, 서버 확장 시 기능 파일 수정 불필요 |
| 2 | DB 충돌 자동 복구 | 동시 접속 시 충돌 반복 | 충돌 발생 시 운영 개입 없이 자동 해결 |
| 3 | 아레나 랭킹 3단계 정렬 | Redis에서 다중 기준 정렬 불가 | DB 추가 조회 없이 Redis 1회 호출로 처리 |
| 4 | 캐시 과부하 방지 | 캐시 만료 시 동시 갱신 폭주 | 순간 과부하 현상 제거, 동일 패턴 2곳 재사용 |
| 5 | 앱 응답 데이터 자동 조립 | push 데이터 누락·중복 | 기능 개발 시 push 별도 처리 불필요, 일관성 보장 |
| 6 | 로그 지연 기록 구조 | 로그 쓰기가 응답 속도에 영향 | 응답 속도 개선, 잘못된 로그 기록 방지 |
| 7 | 프로세스 재사용 상태 초기화 | 이전 요청 상태 오염 | 장기 운영 중 상태 뒤섞임 버그 없음 |
| 8 | 트랜잭션 자동 취소 구조 | 롤백 누락으로 인한 DB 잠금 | 어떤 종료 경로에서도 미완료 트랜잭션 자동 취소 |
| 9 | Redis 다중화 및 장애 전환 | 서버 장애 시 서비스 중단 | 부분 장애 시에도 서비스 무중단 유지 |
| 10 | 전투 스탯 변경 감지 재계산 | 40종 스탯 전체 재계산 낭비 | 불필요한 연산 제거, 속성 간 상호작용 일원화 |
